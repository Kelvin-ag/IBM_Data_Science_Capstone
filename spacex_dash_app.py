...
spacex_df = pd.read_csv('/Users/kelvinagara/Desktop/New PY/PyDataSci/Jupyter/CSV Data/spacex_launch_dash.csv')
...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard', style={'textAlign': 'center', 'color': '#503D36', 'font-size': 40}),
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                             ...
                                             searchable=True), # searchable attribute to be True so we can enter keywords to search launch sites
                                html.Br(),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        fig = px.pie(data, values='class', names='Launch Site', title='Total Successful Launches by All Sites')
        return fig
    else:
        ...
        fig = px.pie(filtered_df, values='Launch Site', names='class', title=f'Total Successful Launches for {entered_site}')
        return fig

# Second callback function for scatter plot inputs
@app.callback(
